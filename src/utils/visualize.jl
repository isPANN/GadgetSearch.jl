...
function plot_graph(g::SimpleGraph, save_path::String; 
    ...
    @pdf begin
        ...
        if pos === nothing
            # Use stress layout if no positions provided
            layout = stress
        else
            # Map and scale the provided positions
            ...
            x_scale_factor, y_scale_factor = _scale_values_with_margin(x_new_vals, y_new_vals, plot_size, plot_size, margin, true)
            
            layout = [Point(x*x_scale_factor, y*y_scale_factor) for (x,y) in zip(x_new_vals, y_new_vals)]
        end
