...
function search_rules_parallel(graph_path, bit_num, gate_list, save_path;
    ...
    # 并行处理每个图块
    Threads.@threads for (chunk_idx, chunk_path) in collect(graph_chunks)
        # 检查是否还有规则需要处理
        lock(rules_lock) do
        ...
        for (i, (gname, graph)) in enumerate(graph_dict)
            ...
            # 打印进度信息
            i % 1000 == 0 && println("Thread $(thread_id): Searched $(i)-th graph in chunk $(chunk_idx)...")
                    ...
                    for output_bits in permutations(remain_elements, output_num)
                        ...
                        for rule_id in collect(local_rules)
                            ...
                            lock(rules_lock) do
                                ...
                                delete!(local_rules, rule_id)
                                println("Thread $(thread_id): Found solution for rule $(rule_id), $(length(remaining_rules)) rules remaining")
                            end
                        end
                    end
                end
