...
constraints = [
    # OR gate: ground states when output=1 for inputs (0,1), (1,0), (1,1), and output=0 for (0,0)
    StateConstraint(["001", "011", "101", "111"]),  # OR-like
    
    # AND gate: ground states when output=1 only for input (1,1)
    StateConstraint(["000", "010", "100", "111"]),  # AND-like
    
    # XOR gate: ground states when output=1 for inputs with odd number of 1s
    StateConstraint(["000", "011", "101", "110"]),  # XOR-like
]
...
for (i, label) in enumerate(labels)
    if !isempty(results[i])
        gadget = results[i][1]
        @info """===== Checking '$label' gadget (QUBO/Full state space) =====
        $(check_gadget_qubo(gadget; _return_info=true))
        """
    end
end
